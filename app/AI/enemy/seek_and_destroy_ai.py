...
class SeekDestroyAI(BaseAI):
    ...
    def _find_closest_friendly_agent_pos(self):
        ...
        for name in agents_name_type_map:
            if (agents_name_type_map[name] == Agent.Friendly) \
                    or (agents_name_type_map[name] == Agent.User_Controlled):
                pos = agents_name_pos_map[name]
                dist = compute_dist(self._pos, pos, self._BLOCK_WIDTH,
                                    self._BLOCK_HEIGHT)
    ...
    def _is_path_clear(self, self_pos, target_pos, action):
        ...
        if action == Actions.MOVE_LEFT:
            for y in range(target_pos[1]+1
                           if target_pos[1] < self_pos[1]
                           else target_pos[1]+1-self._BLOCK_WIDTH,
                           self_pos[1]):
                if blocks_pos_type_map[(self_pos[0], y % self._BLOCK_WIDTH)] \
                        == Block.Excluded:
                    return(False)
        elif action == Actions.MOVE_RIGHT:
            for y in range(self_pos[1]+1, target_pos[1]
                           if self_pos[1] < target_pos[1]
                           else target_pos[1]+self._BLOCK_WIDTH):
                if blocks_pos_type_map[(self_pos[0], y % self._BLOCK_WIDTH)] \
                        == Block.Excluded:
                    return(False)
        elif action == Actions.MOVE_UP:
            for x in range(target_pos[0]+1
                           if target_pos[0] < self_pos[0]
                           else target_pos[0]+1-self._BLOCK_HEIGHT,
                           self_pos[0]):
                if blocks_pos_type_map[(x % self._BLOCK_HEIGHT, self_pos[1])] \
                        == Block.Excluded:
                    return(False)
        elif action == Actions.MOVE_DOWN:
            for x in range(self_pos[0]+1, target_pos[0]
                           if self_pos[1] < target_pos[1]
                           else target_pos[1]+self._BLOCK_HEIGHT):
                if blocks_pos_type_map[(x % self._BLOCK_HEIGHT, self_pos[1])] \
                        == Block.Excluded:
                    return(False)
        return(True)
    ...
    def update(self, is_forced):
        ...
        agents_name_pos_map = self._info.get_agents_name_pos_map()
        if self._current_target_name is None \
                or self._current_target_name not in agents_name_pos_map:
        ...
        self._log.append(
            ['Move Left Score', left_diff,
             'PATH OPEN'
             if self._is_path_clear(self._pos, target_agent_pos,
                                    Actions.MOVE_LEFT)
             else 'PATH BLOCKED'])
        if left_diff > 0 and left_diff < best_action_coord_diff:
            if Actions.MOVE_LEFT in possible_moves \
                    and Actions.MOVE_LEFT not in self._exluded_actions \
                    and self._is_path_clear(self._pos, target_agent_pos,
                                            Actions.MOVE_LEFT):
                best_action = Actions.MOVE_LEFT
                best_action_coord_diff = left_diff
        right_diff = (target_agent_pos[1] - self._pos[1]) % self._BLOCK_WIDTH
        self._log.append(
            ['Move Right Score', right_diff,
             'PATH OPEN'
             if self._is_path_clear(self._pos, target_agent_pos,
                                    Actions.MOVE_RIGHT)
             else 'PATH BLOCKED'])
        if right_diff > 0 and right_diff < best_action_coord_diff:
            if Actions.MOVE_RIGHT in possible_moves \
                    and Actions.MOVE_RIGHT not in self._exluded_actions \
                    and self._is_path_clear(self._pos, target_agent_pos,
                                            Actions.MOVE_RIGHT):
        ...
        self._log.append(
            ['Move Up Score', top_diff,
             'PATH OPEN'
             if self._is_path_clear(self._pos, target_agent_pos,
                                    Actions.MOVE_UP)
             else 'PATH BLOCKED'])
        if top_diff > 0 and top_diff < best_action_coord_diff:
            if Actions.MOVE_UP in possible_moves \
                    and Actions.MOVE_UP not in self._exluded_actions \
                    and self._is_path_clear(self._pos, target_agent_pos,
                                            Actions.MOVE_UP):
        ...
        self._log.append(
            ['Move Down Score', bottom_diff,
             'PATH OPEN'
             if self._is_path_clear(self._pos, target_agent_pos,
                                    Actions.MOVE_DOWN)
             else 'PATH BLOCKED'])
        if bottom_diff > 0 and bottom_diff < best_action_coord_diff:
            if Actions.MOVE_DOWN in possible_moves \
                    and Actions.MOVE_DOWN not in self._exluded_actions \
                    and self._is_path_clear(self._pos, target_agent_pos,
                                            Actions.MOVE_DOWN):
        ...
        if best_action is not None:
            ...
            self._log = [['### Update Log for {0} ###'.
                         format(self._AGENT_NAME)]]

            random_move = random.choice([move for move in possible_moves
                                         if move
                                         not in self._exluded_actions])
    ...
    class PerformanceReport():
        ...
        def update(self, position, new_agents_destroyed):
            """
            Updates the report log based upon the diff between the newly \
                passed info and the old info
            """
            self._positions.append(tuple(position))
            for pair in new_agents_destroyed:
                self._agents_destroyed_timestamps_map[len(self._positions)-1] \
                    = tuple(pair)
        ...
        def _evaluate_performance(self):
            ...
            """
            agents_destroyed_timestamps = \
                list(self._agents_destroyed_timestamps_map)
            if len(agents_destroyed_timestamps) == 0:
                return(0)
            score = (100 / float(self._TOTAL_TARGET_AGENTS)) * 1 \
                / max(1, agents_destroyed_timestamps[0])
            for i in range(1, len(agents_destroyed_timestamps)):
                score += (100 / float(self._TOTAL_TARGET_AGENTS)) * 1 \
                    / max(1, agents_destroyed_timestamps[i] -
                          agents_destroyed_timestamps[i-1])
            return(int(score))
        ...
        def print_to_console(self):
            ...
            """
            print('Agent {0} destroyed {1} targets agents in {2} update \
                  cycles with a score of {3}'.
                  format(self._NAME, self._TOTAL_TARGET_AGENTS,
                         len(self._positions)-1, self._evaluate_performance()))
