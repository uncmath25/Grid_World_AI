...
from ViewModels.game_examples import *


class Game():

    def __init__(self, block_dim, block_width, block_height, background, line_color, line_thickness, screen):
    ...
    def _init_world(self):
        ...
        self._blocks_map, possible_pos = load_blocks_basic(self._BLOCK_WIDTH, self._BLOCK_HEIGHT, 10, 123)
        # self._agents, self._agents_name_radii_map, self._agents_performance_reports_map = load_agents_one_vs_one(self._BLOCK_WIDTH, self._BLOCK_HEIGHT, possible_pos)
        self._agents, self._agents_name_radii_map, self._agents_performance_reports_map = load_agents_snake_mice(self._BLOCK_WIDTH, self._BLOCK_HEIGHT, possible_pos, 3, False)
    ...
    def _init_screen(self, screen):
        ...
        for width in range(self._BLOCK_WIDTH):
            pygame.draw.lines(screen, self._LINE_COLOR, False, ((self._BLOCK_DIM[0]*width, 0), (self._BLOCK_DIM[0]*width, self._BLOCK_DIM[1]*self._BLOCK_HEIGHT)), self._LINE_THICKNESS)
        for height in range(self._BLOCK_HEIGHT):
            pygame.draw.lines(screen, self._LINE_COLOR, False, ((0, self._BLOCK_DIM[1]*height), (self._BLOCK_DIM[0]*self._BLOCK_WIDTH, self._BLOCK_DIM[1]*height)), self._LINE_THICKNESS)
        for row in range(self._BLOCK_HEIGHT):
            for col in range(self._BLOCK_WIDTH):
                self._blocks_map[(row, col)].draw(screen, self._BLOCK_DIM, self._LINE_THICKNESS)
    ...
    def _check_who_died(self, kill_flag_dict):
        ...
        for agent in self._agents:
            if (agent.get_type() == Agent.User_Controlled) or (agent.get_type() == Agent.Friendly):
        ...
        for friendly_agent in friendly_agents:
            for enemy_agent in enemy_agents:
                if friendly_agent.get_pos() == enemy_agent.get_pos():
                    ...
                    destroyed_agent_pairs.append((friendly_agent.get_type(), friendly_agent.get_name()))
    ...
    def draw(self, screen):
        ...
        for pos in self._block_pos_to_redraw:
            self._blocks_map[pos].draw(screen, self._BLOCK_DIM, self._LINE_THICKNESS)
